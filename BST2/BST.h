...
protected:
    ...
    struct BinaryNode
    {
        ...
        BinaryNode(const int &theHeight)
            : height{ theHeight } {}
    ...
    void SingleLeftChild(BinaryNode* & k2){                                         
        if (k2 == nullptr || k2->left == nullptr) return;
    ...
    void SingleRightChild(BinaryNode* & k1){
        if (k1 == nullptr || k1->right == nullptr) return;
    ...
    void DoubleLeftChild(BinaryNode * & k3)
    {
        if(k3 != nullptr)
        {
        SingleRightChild(k3->left);
        SingleLeftChild(k3);
        }
    }
    void DoubleRightChild(BinaryNode * & k1)
    {
        if(k1 != nullptr)
        {
        SingleLeftChild(k1->right);
        SingleRightChild(k1);
        }
    }
    /**
    ...
    while (c != nullptr) { 
        if (x < c->element) {
            ...
            if (c->left != nullptr && c->right != nullptr) { 
                BinaryNode *minNode = detachMin(c->right); // 分离右子树的最小节点
                minNode->left = c->left;                   
                minNode->right = c->right;                
                if (p == nullptr) { // 删除的是根节点
                    t = minNode;
                } else if (p->left == c) {
                    p->left = minNode; // 将最小节点连接到父节点的左侧
                } else {
                    p->right = minNode; // 将最小节点连接到父节点的右侧
                }
                delete c;  
            } else { // 只有一个子节点或无子节点
